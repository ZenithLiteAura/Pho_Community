...
import 'package:img_syncer/core/sync/background_runner.dart';
import 'package:img_syncer/bridge/notifications/local_notifier.dart';

export 'package:img_syncer/core/sync/background_runner.dart'
...
class SyncBodyState extends State<SyncBody> {
  ...
  Widget columnBuilder(BuildContext context, StateModel model, Widget? child) {
    ...
    final all = assetModel.localAssets;
    // 先过滤出需要展示的资产（与同步计数共用同一判定逻辑），
    // 再交给 ListView.builder 按需构建，避免一次性构建全部 ListTile。
    final List<Asset> visible = [];
    for (var asset in all) {
      ...
      body: Column(
        ...
        children: [
          ...
          model.refreshingUnsynchronized && visible.isEmpty
              ? Container(
                  height: 250,
                  padding: const EdgeInsets.symmetric(horizontal: AppSpacing.xl),
                  child: Center(
              ...
              : Flexible(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildAssetRow(context, visible[index], index),
                  )),
        ],
      ),
    );
  ...
  /// 构建单行未同步资产（由 ListView.builder 按需调用）。
  ///
  /// 原先在 columnBuilder 里一次性构建全部 ListTile，配合高频进度通知会造成
  ...
  Widget _buildAssetRow(BuildContext context, Asset asset, int index) {
    ...
    return ListTile(
      ...
      subtitle: needLoadThumbnail
          ? _buildProgressSubtitle(context, asset)
          : Container(),
    );
  }
  ...
  /// 进度 / 状态副标题。
  ///
  /// 只监听 [StateModel.uploadProgressTick]（高频、已节流），因此上传过程中
  /// 高频的进度刷新只重绘这一小块，不再带动整个同步页与整张列表重建 ——
  /// 这是「进度条抽搐」的根治点。
  Widget _buildProgressSubtitle(BuildContext context, Asset asset) {
    return ListenableBuilder(
      listenable: stateModel.uploadProgressTick,
      builder: (context, _) {
        final id = asset.local!.id;
        final percent = stateModel.getUploadPercent(id);
        if (percent > 0) {
          final state = stateModel.uploadProgress[id];
          final retry = state?.retry ?? 0;
          final maxRetries = state?.maxRetries ?? 0;
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // 重试中：明确告知，避免用户以为进度条在莫名回退
                if (retry > 0 && maxRetries > 0)
                  Padding(
                    padding: const EdgeInsets.only(top: 2),
                    child: Text(
                      '${l10n.retrying} ($retry/$maxRetries)',
        ...
        if (!stateModel.syncedIDs.contains(id)) {
          if (uploadFailedMap.containsKey(id)) {
            return Text(
              "${l10n.uploadFailed}: ${uploadFailedMap[id]}",
