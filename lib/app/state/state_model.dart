...
/// 由 prefs 中保存的存储类型字符串还原 [Drive] 枚举；未知值回退 SMB。
/// 统一定义在此处，避免各页面重复实现（原 storage_config_page 与
/// setting_storage_route 各有一份同名实现）。
...
class SettingModel extends ChangeNotifier {
  ...
  /// 并行上传数（>1 时多文件并发上传）。
  /// 手动同步与后台同步共用此值；由 global.dart 启动时从
  /// `uploadParallelCount` 偏好加载，UI 修改后立即生效。
  /// 默认 6 路并发以充分利用带宽；老用户保留原 SharedPreferences 值。
  int paralleUploadCount = 6;

  /// 文件筛选器开关（占位）：Pro 版功能，开源版恒为 false。
  /// 保留该字段以与既有测试对齐。
  bool enableFilter = false;

  bool setParallelUploadCount(int count) {
    if (paralleUploadCount == count) return false;
    paralleUploadCount = count;
    notifyListeners();
    return true;
  }
...
class transmitState {
  ...
  /// 当前处于第几次重试（0 表示未重试）。
  /// 供 UI 显示「重试中 (n/max)」，让用户理解进度为何回到起点，
  /// 而不是看到进度条莫名跳变/闪烁。
  int retry = 0;
  int maxRetries = 0;
}
...
/// 可被外部显式触发的高频通知源。
///
/// `ChangeNotifier.notifyListeners()` 为 protected，无法在类外调用，
/// 因此用这个薄子类把「进度值变化」这类高频通知独立出来：
/// 进度条自身监听它，而同步页的整体重建不因高频进度而触发。
class ProgressTicker extends ChangeNotifier {
...
class StateModel extends ChangeNotifier {
  ...
  /// 已上传文件的采样哈希集合（内容级去重，跨启动/跨 id 变化仍有效）。
  Set<String> uploadedHashes = {};

  /// 已上传文件的名称集合（服务器端存储名，防哈希不可算时仍可去重）。
  ...
  /// 高频进度刷新专用通知源（与整体重建解耦）。
  ///
  /// 上传进度按 HTTP 分块粒度更新（`file.openRead()` 默认 64KB 一块），
  /// 1GB 视频会产生上万次回调；若每次都 notifyListeners()，同步页整个列表
  /// （含全部 ListTile）会被反复重建，表现为进度条抽搐。故把两类通知分开：
  ///  - **进度值变化** → 只 tick 本 notifier，仅进度条自身重绘；
  ///  - **进入/退出上传**（集合空↔非空）、**单文件完成** → 仍 notifyListeners()，
  ///    因为底部 FAB 可用状态与列表项文案依赖 isUploading() / syncedIDs。
  final ProgressTicker uploadProgressTick = ProgressTicker();
  final ProgressTicker downloadProgressTick = ProgressTicker();

  /// 同一文件进度通知的最小间隔（节流），避免 64KB 粒度的刷新风暴。
  static const int progressThrottleMs = 120;

  final Map<String, int> _lastUploadTickMs = {};
  final Map<String, int> _lastDownloadTickMs = {};
  ...
  void updateUploadProgress(String id, int transmitted, int total) {
    final wasEmpty = uploadProgress.isEmpty;
    ...
    // 进入上传（空 → 非空）：需要整体通知，让 FAB 禁用等状态即时生效。
    if (wasEmpty) {
      notifyListeners();
      return;
    }
    // 节流：同一文件最快每 progressThrottleMs 刷一次；传完时立即刷，避免结束态被延迟。
    final now = DateTime.now().millisecondsSinceEpoch;
    if (transmitted < total &&
        now - (_lastUploadTickMs[id] ?? 0) < progressThrottleMs) {
      return;
    }
    _lastUploadTickMs[id] = now;
    uploadProgressTick.tick();
  }

  /// 标记某文件正在重试。
  ///
  /// 要点：**保留进度条本身**（不 remove），只把本轮进度归零并记录重试次数，
  /// 由 UI 显示「重试中 (n/max)」。原先的重试会在 catch 里直接 finishUpload，
  /// 导致进度条被移除、重试时又重建 —— 并行上传时表现为两条进度条交替闪烁。
  void markUploadRetrying(String id, int retry, int maxRetries) {
    final state = uploadProgress[id];
    if (state == null) return;
    state.retry = retry;
    state.maxRetries = maxRetries;
    state.transmitted = 0; // 本轮从头发送，如实归零而非静默回退
    uploadProgressTick.tick();
  }

  void finishUpload(String id, bool success) {
    uploadProgress.remove(id);
    _lastUploadTickMs.remove(id);
    if (success) {
      syncedIDs.add(id);
      saveSyncedIDs();
    }
    // 单文件完成会改变列表项文案（已上传/失败）与 FAB 可用性；
    // 频率为「每文件一次」，整体通知可以接受。
    notifyListeners();
  }

  void updateDownloadProgress(String id, int transmitted, int total) {
    final wasEmpty = downloadProgress.isEmpty;
    ...
    final now = DateTime.now().millisecondsSinceEpoch;
    if (transmitted < total &&
        now - (_lastDownloadTickMs[id] ?? 0) < progressThrottleMs) {
      return;
    }
    _lastDownloadTickMs[id] = now;
    downloadProgressTick.tick();
  }

  void finishDownload(String id, bool success) {
    downloadProgress.remove(id);
    _lastDownloadTickMs.remove(id);
    notifyListeners();
  }

  /// 上传进度（0.0 - 1.0）。
  ///
  /// 对 total 为 0 / 异常值做防御：LinearProgressIndicator 的 value 必须是
  /// [0,1] 内的有限值，否则会触发断言，表现为进度条绘制异常。
  double getUploadPercent(String id) {
    final state = uploadProgress[id];
    if (state == null) return 0;
    return _safePercent(state.transmitted, state.total);
  }
  ...
  static double _safePercent(int transmitted, int total) {
    if (total <= 0) return 0;
    final v = transmitted / total;
  ...
  /// 从偏好加载已上传哈希集合。
  Future<void> loadUploadedHashes() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonString = prefs.getString('uploaded_hashes');
    if (jsonString == null) return;
    try {
      final list = jsonDecode(jsonString);
      if (list is List) {
        uploadedHashes = list.map((e) => e.toString()).toSet();
      }
    } catch (e) {
      uploadedHashes = {};
    }
  ...
  /// 持久化已上传哈希集合。
  Future<void> saveUploadedHashes() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('uploaded_hashes', jsonEncode(uploadedHashes.toList()));
  }

  /// 记录一个已上传文件的哈希。
  Future<void> recordUploadedHash(String hash) async {
    if (uploadedHashes.add(hash)) {
      await saveUploadedHashes();
    }
  }

  /// 判断某文件哈希是否已上传过（内容级去重）。
  bool isHashUploaded(String hash) => uploadedHashes.contains(hash);

  /// 从偏好加载已上传名称集合。
  Future<void> loadUploadedNames() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonString = prefs.getString('uploaded_names');
    if (jsonString == null) return;
    try {
      final list = jsonDecode(jsonString);
      if (list is List) {
        uploadedNames = list.map((e) => e.toString()).toSet();
      }
    } catch (e) {
      uploadedNames = {};
    }
  ...
  /// 持久化已上传名称集合。
  Future<void> saveUploadedNames() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('uploaded_names', jsonEncode(uploadedNames.toList()));
  }

  /// 记录一个已上传文件的服务器存储名。
  Future<void> recordUploadedName(String name) async {
    if (uploadedNames.add(name)) {
      await saveUploadedNames();
    }
  }

  /// 判断某文件名是否已上传过（名称级去重）。
  bool isNameUploaded(String name) => uploadedNames.contains(name);

  /// 从偏好加载已上传路径集合。
  Future<void> loadUploadedPaths() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonString = prefs.getString('uploaded_paths');
    if (jsonString == null) return;
    try {
      final list = jsonDecode(jsonString);
      if (list is List) {
        uploadedPaths = list.map((e) => e.toString()).toSet();
      }
    } catch (e) {
      uploadedPaths = {};
    }
  ...
  /// 持久化已上传路径集合。
  Future<void> saveUploadedPaths() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('uploaded_paths', jsonEncode(uploadedPaths.toList()));
  }

  /// 记录一个已上传文件的本地路径。
  Future<void> recordUploadedPath(String path) async {
    if (uploadedPaths.add(path)) {
      await saveUploadedPaths();
    }
  }

  /// 判断某本地路径是否已上传过（路径级去重）。
  bool isPathUploaded(String path) => uploadedPaths.contains(path);
}
...
Future<void> scanFile(String filePath) async {
  if (Platform.isAndroid) {
    try {
      ...
      await const MethodChannel('com.ZenithLiteAura.app.pho/RunGrpcServer')
...
Future<void> refreshUnsynchronizedPhotos() async {
  ...
  if (!assetModel.refreshUnsynchronizedNeedBreak) {
    // 合并而非整体替换：保留本地已记录的已同步 ID（服务器比对可能因
    // 文件名编码/索引延迟漏掉刚上传的照片），防止已上传照片被重新上传。
    // Set 去重后写入。
    final merged = {...stateModel.syncedIDs, ...newSyncedIDs}.toList();
