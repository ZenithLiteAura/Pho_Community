...
class GalleryBodyState extends State<GalleryBody>
  ...
  int columCount = 3; // Fluid Horizon 3-column grid
  ...
  void _shareAsset() async {
    ...
    final all =
        _useLocal ? assetModel.localAssets : assetModel.remoteAssets;
    final assets = <Asset>[];
  ...
  void downloadSelected() async {
    if (_useLocal || !stateModel.isSelectionMode) {
      return;
    }
      ...
      builder: (context, model, child) {
        ...
        return SliverAppBar(
          ...
          actions: [
            ...
            IconButton(
              icon: Icon(
                _useLocal ? Icons.cloud_outlined : Icons.phone_android_outlined,
                color: AppColors.primary,
                size: 22,
              ),
              tooltip: _useLocal ? l10n.cloud : l10n.local,
              onPressed: () {
                setState(() {
                  _useLocal = !_useLocal;
                });
  ...
  Widget contentBuilder(BuildContext context, AssetModel model, Widget? child) {
    ...
    for (int i = 0; i < all.length; i++) {
      ...
      if (currentDateTime == null ||
        ...
        // Use a grid layout for 3-column squircle grid
        children.add(Padding(
          padding: EdgeInsets.symmetric(horizontal: gridSpacing),
          child: Wrap(
            spacing: gridSpacing, // Main axis (horizontal) spacing
            runSpacing: gridSpacing, // Cross axis (vertical) spacing
        ...
        currentScrollOffset -= gridSpacing;
        ...
        if (currentDateTime == null || date.month != currentDateTime.month) {
          // Today's date grouping title with Fluid Horizon styling
      ...
      var child = GestureDetector(
          ...
          child: Stack(
            children: [
              // image with squircle corners (AppRadius.card = 24px)
              Container(
                  width: imgWidth,
                  height: imgHeight,
                  ...
                  child: Hero(
                    ...
                    child: _ThumbnailLoader(
                      asset: all[i],
                      enablePreload: needLoadThumbnail,
                    ),
                      ...
                      return Positioned(
                        ...
                        child: Text(
                          ...
                          style: textTheme.labelMedium
                              ?.copyWith(color: colorScheme.onPrimary), // Use theme color
                        ),
                      );
                    }),
                Positioned(
              ...
              // selection with squircle corners
              if (stateModel.isSelectionMode) ...[
      ...
      if (currentChildren.length % columCount == 1) {
        currentScrollOffset += imgHeight + gridSpacing;
      }
    ...
    return SliverList.list(
      children: [
        ...
        if (all.isEmpty &&
            ((_useLocal && assetModel.localGetting == null) ||
                (!_useLocal && assetModel.remoteGetting == null)))
          SizedBox(
            height: 300,
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _useLocal ? l10n.noLocalPhotos : l10n.noCloudPhotos,
                    style: textTheme.bodyMedium
                        ?.copyWith(color: colorScheme.onSurfaceVariant),
                  ),
                  const SizedBox(height: 16),
                  if (!_useLocal && !settingModel.isRemoteStorageSetted)
                    FilledButton(
  ...
  return Container(
    ...
    child: IconButton.filledTonal(
      ...
      onPressed: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const StorageConfigPage()),
        );
      },
    ),
  );
...
/// v3.1：缩略图自加载 Widget。
///
/// 加载完成后仅 setState 自身（~1 个 cell），不触发父级 GalleryBody 全页重建。
class _ThumbnailLoader extends StatefulWidget {
  const _ThumbnailLoader({
    required this.asset,
...
class _ThumbnailLoaderState extends State<_ThumbnailLoader> {
  ...
  void initState() {
    super.initState();
    _ready = widget.asset.loadThumbnailFinished();
    if (!_ready && widget.enablePreload) {
  ...
  @override
  void didUpdateWidget(covariant _ThumbnailLoader oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return const ThumbnailSkeleton();
  }
}
