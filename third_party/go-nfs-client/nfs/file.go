...
// It returns -1 if the size cannot be determined (e.g. RPC failure),
// letting the caller fall back to a Lookup/Stat round-trip.
func (f *File) FileSize() int64 {
	...
	if err != nil {
		util.Debugf("getattr(%x): %s", f.fh, err.Error())
		return -1
	}

	res := &GetAttrRes{}
	if err = xdr.Read(r, res); err != nil {
		util.Debugf("getattr(%x): decode: %s", f.fh, err.Error())
		return -1
	}
	return res.ObjAttr.Size()
}
