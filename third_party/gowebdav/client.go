...
// ReadStreamSized reads the stream for a given path, returning the response
// body together with its size in bytes. size is -1 when the server omits
// Content-Length (the caller can fall back to Stat).
func (c *Client) ReadStreamSized(path string) (io.ReadCloser, int64, error) {
	return c.readStreamSizedImpl(path, 0, -1, false)
}

// ReadStreamRangeSized reads a byte range of a file for a given path,
// returning the response body together with its size in bytes.
// The range is expressed as offset from the start of the file and length;
// length < 0 means "to the end of file". size is -1 when the server omits
// Content-Length (the caller can fall back to Stat).
func (c *Client) ReadStreamRangeSized(path string, offset, length int64) (io.ReadCloser, int64, error) {
	return c.readStreamSizedImpl(path, offset, length, true)
}

func (c *Client) readStreamSizedImpl(path string, offset, length int64, useRange bool) (io.ReadCloser, int64, error) {
	...
	if err != nil {
		return nil, 0, newPathErrorErr("ReadStream", path, err)
	}
	...
	rs.Body.Close()
	return nil, 0, fmt.Errorf("ReadStream %s: %s", path, body)
}
