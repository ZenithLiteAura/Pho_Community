...
import (
	...
	"github.com/fregie/img_syncer/core/imgmanager"
)
...
const (
	...
	// maxUploadSize 是**单个上传请求体**的上限（主文件 / 缩略图 / Live 视频各自独立计算）。
	//
	// 该值同时经 Ping 下发给客户端，客户端据此在上传前预检，
	// 从而避免「客户端已开始上传、服务端读到上限后中途断连」的情况。
	//
	// 放宽到 8GiB：手机 4K 视频约 300-400MB/分钟，原先的 500MiB 很容易被撞到。
	// 超限时 Go 的 http server 会因请求体未读完而直接关闭连接，客户端只能看到
	// Broken pipe，收不到 413 —— 这正是「大视频上传失败：Broken pipe」的成因。
	maxUploadSize = 8 << 30 // 8GiB
)
